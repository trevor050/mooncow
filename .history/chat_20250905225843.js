...
- Tool: multi_source_search
  ...
  - Parameters:
    ...
    - Call it by emitting a tool call when you need external factual context. After tool results are returned, write a normal helpful answer (do NOT produce another tool call unless new data is still required). Do not include tool call formatting in the final user-facing answer.
  - When to call:
 ...
 */
export async function getCerebrasCompletion(messages, options = {}) {
    ...
    try {
                // Always enable tools (imported) for this model loop
                bodyBase.tools = openAITools;
                bodyBase.tool_choice = 'auto';
                bodyBase.parallel_tool_calls = false;

                let loopGuard = 0;
                while (true) {
                        console.log('[CerebrasChat] Requesting completion (loop', loopGuard, ')');
                        ...
                        if (toolCalls.length === 0 && typeof msg.content === 'string') {
                            ...
                            if (parsed) {
                                const fauxId = crypto?.randomUUID?.() || `tool_${Date.now()}`;
                                bodyBase.messages.push({ role: 'assistant', content: '', tool_calls: [{ id: fauxId, type: 'function', function: { name: parsed.name, arguments: JSON.stringify(parsed.arguments) } }] });
                                const result = await executeToolCall({ name: parsed.name, arguments: JSON.stringify(parsed.arguments) });
                                bodyBase.messages.push({ role: 'tool', tool_call_id: fauxId, name: parsed.name, content: JSON.stringify(result) });
                        ...
                        if (toolCalls.length === 0) {
                            let content = sanitizeThinkContent(msg.content || '');
                            return content.trim();
                        }

                        // Append sanitized assistant turn
                        bodyBase.messages.push({ role: 'assistant', content: msg.content ?? '', tool_calls: msg.tool_calls });
                        for (const call of toolCalls) {
                            const result = await executeToolCall({ name: call.function?.name, arguments: call.function?.arguments });
                            bodyBase.messages.push({ role: 'tool', tool_call_id: call.id || (crypto?.randomUUID?.() || String(Date.now())), name: call.function?.name, content: JSON.stringify(result) });
                        }
                        if (++loopGuard > 6) throw new Error('Tool loop exceeded 6 turns');
        }
 ...
 */
export async function* streamCerebrasCompletion(messages, options = {}) {
...
function sanitizeThinkContent(text) {
    ...
    let t = text;
    t = t.replace(/<think>[\s\S]*?<\/think>/gi, ''); // remove full blocks
    t = t.replace(/<\/?:think>/gi, ''); // remove any stray open/close tags leftover
    return t.trim();
}
