// Tool registry (loaded globally via background scripts). No ESM import for Firefox MV3.
const openAITools = self.openAITools || self.openAITools?.value || [];
const executeToolCall = self.executeToolCall;

// Debug flag for verbose tool call inspection
const DEBUG_TOOL_CALLS = true;
function debugTool(...args) { if (DEBUG_TOOL_CALLS) console.log(...args); }
...
- Tool: cerebras_search
  - Purpose: Search Cerebras Inference documentation for technical information, API references, and implementation guidance.
  - Parameters:
    - query: string. Specific technical terms or concepts related to Cerebras AI services.
  - When to call:
    - Questions about Cerebras AI, inference, models, API usage, tool calling, or development with Cerebras services.
    - Need official documentation or implementation details for Cerebras features.
  - When NOT to call:
    - General AI questions not specific to Cerebras services.
  - Examples:
    - User: "How does Cerebras tool calling work?" → call with { query: "tool calling function calling" }
    - User: "What models does Cerebras support?" → call with { query: "available models API" }
...
async function getCerebrasCompletion(messages, options = {}) {
    ...
    try {
        // Use tools if available (tool-registry.js) - better check for tool availability
    const toolsEnabled = Array.isArray(openAITools) && openAITools.length > 0 && typeof executeToolCall === "function";
        if (toolsEnabled) {
            bodyBase.tools = openAITools;
            bodyBase.tool_choice = 'auto'; // Change to 'required' for debugging if needed
            bodyBase.parallel_tool_calls = false; // Recommended by Cerebras docs
        }

        let loopGuard = 0;
        while (true) {
            debugTool('[CerebrasChat] Requesting completion (loop', loopGuard, ') toolsEnabled=', !!toolsEnabled);
            if (loopGuard === 0) {
                // Log initial payload without API key
                try { debugTool('[CerebrasChat] Outgoing payload (sans key):', JSON.stringify({ ...bodyBase }, null, 2)); } catch(_) {}
            }
            ...
            debugTool('[CerebrasChat] Raw assistant message:', JSON.stringify(msg, null, 2));
            if (!msg) throw new Error('No message in completion');

            let toolCalls = msg.tool_calls || [];
            if (toolCalls.length) {
                debugTool('[CerebrasChat] tool_calls array:', JSON.stringify(toolCalls, null, 2));
            }
            ...
            if (toolCalls.length === 0 && typeof msg.content === 'string') {
                ...
                if (parsed) {
                    debugTool('[CerebrasChat] Parsed textual tool call fallback:', parsed.name, parsed.arguments);
                    if (typeof executeToolCall !== 'function') {
                        throw new Error('Tool executor unavailable (executeToolCall not loaded)');
                    }
                    ...
                    const result = await executeToolCall({ name: parsed.name, arguments: JSON.stringify(parsed.arguments) });
                    debugTool('[CerebrasChat] Tool execution result (fallback):', result);
                    ...
                    debugTool('[CerebrasChat] Conversation after fallback tool result:', JSON.stringify(bodyBase.messages.slice(-4), null, 2));
            ...
            if (!toolsEnabled || toolCalls.length === 0) {
                ...
                debugTool('[CerebrasChat] Full API Response (no tool calls):', data);
                return content.trim();
            }
            ...
            for (const call of toolCalls) {
                ...
                debugTool('[CerebrasChat] Executing tool:', toolName, argsJson);
                ...
                debugTool('[CerebrasChat] Tool result message:', toolMsg);
                bodyBase.messages.push(toolMsg);
                debugTool('[CerebrasChat] Conversation tail after tool result:', JSON.stringify(bodyBase.messages.slice(-6), null, 2));
            }
