...
(function installFetchTextTruncation() {
    try {
        ...
        console.warn('[Mooncow] Could not install fetch text truncation:', e);
    }
})();
                ...
                if (tokenObj.type === 'thought') {
                    ...
                    try {
                        ...
                        if (tokenObj.blob && typeof tokenObj.blob === 'string') {
                            ...
                            try {
                                const res = tokenObj.result || {};
                                // If results array exists, show counts per source bucket for quick verification
                                if (Array.isArray(res.results)) {
                                    ...
                                    console.log('multi_source_search: source-bucket counts:', totals);
                                } else if (res.sources && typeof res.sources === 'object') {
                                    console.log('multi_source_search: top-level sources keys:', Object.keys(res.sources));
                                }
                            } catch (_) {}
                        }
                    } catch (_) {}
