...
self.MultiSourceSearchTool = {
  ...
  async execute({ queries, includeCoding=false, codingRelated=false, client_profile=null, includeNews_Current_Events=false, includeLegal_Gov=false, includeResearch_Scholarly=false, includeSocial_Dev=false, includeOpen_Data_Stats=false, includeArchives_Provenance=false, includeLocation_Geo=false, includeJinaSearch=false }) {
    ...
    // Auto expansion heuristic: if focused on Trump & defense topics and we have room (<5), add related canonical policy queries
    try {
      if (Array.isArray(queries) && queries.length < 5) {
        const lowerAll = queries.join(' || ').toLowerCase();
        if (/trump/.test(lowerAll) && /(defense|pentagon|military)/.test(lowerAll)) {
          const expansions = [
            'Trump Space Force 2019',
            'Trump transgender military policy',
            'Trump NDAA 2019 Section 1245',
            'Trump Syria troop withdrawal 2019'
          ];
            for (const e of expansions) {
              if (queries.length >= 5) break;
              if (!queries.some(q => q.toLowerCase() === e.toLowerCase())) queries.push(e);
            }
        }
      }
    } catch(_) {}
  ...
  const DEFAULT_TIMEOUT = 8000; // bump slightly to reduce news empty results
    const PER_SOURCE = 5;
    ...
    const results = await Promise.all((queries || []).map(async (q) => {
      ...
      let wikiFull = null;
      try {
        // If summary missing or disambiguation, attempt opensearch to locate canonical title
        if (!wiki || String(wiki.type||'').includes('disambiguation')) {
          const openSearch = await getJson(`https://en.wikipedia.org/w/api.php?action=opensearch&search=${encodeURIComponent(q)}&limit=1&namespace=0&format=json&origin=*`);
          if (Array.isArray(openSearch) && openSearch[1] && openSearch[1][0]) {
            const fallbackTitle = openSearch[1][0];
            const alt = await getJson(`https://en.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(fallbackTitle)}`);
            if (alt && !String(alt.type||'').includes('disambiguation')) wiki = alt;
          }
        }
      ...
      if (includeNews_Current_Events) {
        const [googleNewsXml, reutersXml, guardianXml, apXml, dodXml, whiteHouseXml] = await Promise.all([
          getText(`https://news.google.com/rss/search?q=${encodeURIComponent(q)}&hl=en-US&gl=US&ceid=US:en`),
          getText(`https://www.reuters.com/rss/world`),
          getText(`https://www.theguardian.com/world/rss`),
          getText(`https://apnews.com/hub/ap-top-news?output=rss`),
          getText(`https://www.defense.gov/DesktopModules/ArticleCS/RSS.ashx?ContentType=1&Site=945`),
          getText(`https://www.whitehouse.gov/feed/`)
        ]);
        sources.news_current_events = {
          ...
          defense_gov: parseRssAtom(dodXml, PER_SOURCE),
          white_house: parseRssAtom(whiteHouseXml, PER_SOURCE)
        };
      }
      ...
      if (includeArchives_Provenance) {
        ...
        // Refined Internet Archive search: prefer textual / web documents relevant to defense policy.
        const iaQuery = `(${q.replace(/\s+/g,' ')}) AND (defense OR pentagon OR military)`;
        const iaSearch = await getJson(`https://archive.org/advancedsearch.php?q=${encodeURIComponent(iaQuery)}&fl%5B%5D=identifier&fl%5B%5D=title&rows=25&page=1&output=json`);
        if (looksUrl) {
